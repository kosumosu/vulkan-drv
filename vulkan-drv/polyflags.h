...
\note Texture caching/conversion functions should be passed explicit polyflags. This is because the flags in TextureInfo structures aren't correct; the ones passed to
PrecacheTexture() and DrawXXXX() calls are.

\note For the Direct3D 11 renderer, flags are sent with each vertex. This way, if possible, flags can be handled by the shaders without incurring any state changes and/or draw calls.
As such, flag handling is spread over the renderer and shader. Flags that change blend or depth state are not handled in the shader.

Furthermore, flags are also used in texture conversion.

Flag interpretation:

- (flags & PF_Modulated) Switch to modulation blending. Do not draw using a color. DEST_COLOR, SRC_COLOR.
- (flags & PF_Translucent) Switch to translucent blending. Do not alpha test. ONE, INV_SRC_COLOR.
- (flags & PF_Highlight) Switch to highlight blending. For UnrealEd (???). ONE, INV_SRC_ALPHA.
- (flags & PF_AlphaBlend) Switch to alpha blending. Rune only. SRC_ALPHA, INV_SRC_ALPHA.
- (flags & PF_Invisible) Do not draw; however, cannot just discard as depth needs to be set (example: DX NYC generator computer office). ZERO, ONE.

If neither of these, no blending is used.

- (flags & PF_Occlude) Enable depth writes. 
- !(flags & (PF_Translucent|PF_Modulated)) Enable depth writes. I.e. not having these blendstates means occlusion.
...
- (flags & (PF_RenderFog|PF_Translucent|PF_Modulated|PF_AlphaBlend))==PF_RenderFog) Draw the model's fog color.
- (flags & PF_Masked) Replace palette entry 0 by transparent black. Draw using alpha testing (a<0.5). However, do not alpha test if PF_Translucent or PF_AlphaBlend
- (flags & PF_NoSmooth) Sample texture with no filtering. Used for UI elements etc.
*/
...
#define		PF_AlphaBlend		 0x00001000	/**< RUNE:  This poly should be alpha blended. NOTE: as this uses the game-specific flag number, only check this for Rune */
